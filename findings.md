...
### Submitted
2026-08-17: camera-ready uploaded to OpenReview (MLSP 2026, accept as poster, in proceedings). Final text: paper/main.tex at the commit tagged for the submission; 6 pages on Overleaf; abstract as in the paper; author block Syed (FIT), Silaghi (FIT), Abujar (UAB), Akter Khushbu (DIU); copyright notice 4 from the 2026 template. Last pre-submission corrections (same day): head-to-head failure statement rewritten from the pair data (7 of 11 failures are near ties under 10%, 2 within 25%, 2 clear misses), the denoising energy ratios corrected to the measured 8 to 28x at N <= 1024 and within 10% at N >= 4096, the parallel-regime and S_o wording made exact. Post-submission: make github.com/jemsbhai/TOMLSignals public and add the reproduction section to its README.
